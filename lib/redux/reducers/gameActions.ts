...
export const handleForwardThunk = (playerNo: number, id: string, pos: number) => {
    return async (dispatch: ApplicationDispatch, getState: () => RootState) => {
        ...
        const beforePlayerPiece = state.game[`player${playerNo}`].find((e: PLAYER_PIECE) => e.id === id);
        let travelCount = beforePlayerPiece?.travelCount!;

        for (let i = 0; i < diceNo; i++) {
            const updatedPosition = getState();
            const playerPiece = updatedPosition.game[`player${playerNo}`].find((e: PLAYER_PIECE) => e.id === id);
        ...
        const finalPlot = updatedPlottedPieces.filter((e: any) => e.pos == finalPath);
        const ids = finalPlot.map((e: any) => e.id[0]);
        ...
        if (
            ...
            const enemyPiece = finalPlot.find((p: any) => p.id[0] !== id[0]);
